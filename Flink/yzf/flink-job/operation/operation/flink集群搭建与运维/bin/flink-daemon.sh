...
case $STARTSTOP in
    ...
    (start)
        ...
        # This code is modified by yangchengkai@yunzhangfang.com.
        # See more on gitlab issue: http://gitlab.yzf.net/group_di/bigdata/flink-job/issues/48
        $JAVA_RUN $JVM_ARGS ${FLINK_ENV_JAVA_OPTS} "${log_setting[@]}" -classpath "`manglePathList "$FLINK_TM_CLASSPATH:$INTERNAL_HADOOP_CLASSPATHS"`" ${CLASS_TO_RUN} "${ARGS[@]}" > /dev/null 200<&- 2>&1 < /dev/null &
        mypid=$!
